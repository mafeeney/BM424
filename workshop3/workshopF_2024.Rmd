...
The cause of the outbreak appears to be <i>Neisseria meningitidis</i>. (Please refer to the [CDC Meningococcal Disease Fact Sheet](https://www.cdc.gov/meningococcal/downloads/17-275138A-MeningococcalDis-FS.pdf) for more information about the disease.)
...
knitr::include_graphics(rep("images/Neisseria.jpg"))
```
...
9.  Epidemiologist from the local university, an expert on <i>N. meningitidis</i> transmission
...
```{r city, echo=FALSE, fig.cap="Photograph of Glenmarigg city centre. Image credit: DALL-E"}
knitr::include_graphics(rep("images/Glenmarigg.png"))
```
...
```{r A1, fig.show="hold", out.width="50%", echo=FALSE, fig.cap="Glenmarigg demographics (Source: Office for National Statistics)"}
...
ggplot(data = population, aes(x=PopYears, y=Pop)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of Glenmarigg residents (estimated)", x = "Year") +
...
ggplot(data = birthdeath, aes(x=DemogYears, y=Num, group=Demog, colour=Demog)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of births and deaths in Glenmarigg (estimated)", x = "Year", colour = "Demographics") +
  scale_y_continuous(limits=c(0, 5500)) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_color_discrete(name="")
```
You were elected as a member of the popular "Community Change" party and made a number of election promises regarding a "Green New Deal" for `r city_name`.

```{r A2, fig.cap="Voting intention by Glenmarigg residents (Source: YouGov polling)", echo=FALSE}

Party <- c((rep("Moderate", 10)), (rep("Progress and Peace", 10)), (rep("Community Change", 10)), (rep("Aquamarine", 10)), (rep("Modern Union", 10)))
...
Parties <- c("Moderate", "Progress and Peace", "Community Change", "Aquamarine", "Modern Union")
...
```{r A4, fig.cap="Public expenditures by category for the 2021-2022 financial year (Source: Glenmarigg City Council)", echo=FALSE}
...
business2 <- c("The Glenmarigg Shop", "Fraser & Co.", "Bella's", "Around the Corner", "Jewels")
...
You are a representative from Public Health Scotland, sent to `r city_name` to help with the <i>Neisseria</i> outbreak. 
...
You are an epidemiologist working for Public Health Scotland, sent to `r city_name` to help with the <i>Neisseria</i> outbreak.

A team of scientists working together have in a PHS lab have isolated and cultured <i>Neisseria</i> (following standard procedures) from a number of patient samples taken during this outbreak; isolated DNA from these cultures; sequenced this DNA using an Illumina MiSeq platform; assembled the short paired-end reads; and compared these assemblies to the extant <i>Neisseria</i> sequences available in the NCBI databases. The results of these experiments are summarised in the table below. 
...
```{r hospital, echo=FALSE, fig.cap="The Royal Jubilee hospital in Glenmarigg. Image credit: [DALL-E"}
...
```{r A6, fig.cap="ICU bed occupancy for the Glenmarigg Royal Jubilee hospital (Source: NHS Scotland)", echo=FALSE}
...
ggplot(data = ICUstats, aes(x=ICUdates, y=ICU, group = 1)) + 
  geom_point() +
  geom_line() +
  labs(y = "Average Royal Jubilee ICU bed occupancy/month", x = "Month") +
...
```{r A7, fig.cap="Admissions to the Glenmarigg Royal Jubilee hospital by category (Source: NHS Scotland)", echo=FALSE}
...
```{r A8, fig.cap="ICU waiting times for the Glenmarigg Royal Jubilee hospital (Source: NHS Scotland)", echo=FALSE}
...
```{r A9, fig.cap="Key Performance Indicators (KPIs) related to infectious diseases for the Glenmarigg Royal Jubilee hospital (Source: NHS Scotland)", echo=FALSE}
...
There are currently `r sample(143:180, size=1)` patients in the `r hospital_name` with confirmed or suspected meningococcal disease, with `r sample(15:17, size=1)` of these patients currently in the ICU. 
...
<table border="1">
  ...
  <tr>
    <td>Schools</td>
    <td>`r sample(2:8, size=1)` primary schools, `r sample(2:5, size=1)` secondary schools, 2 universities</td>
  </tr>
  <tr>
    <td>Care Homes</td>
...
Your briefing packet on <i>Neisseria</i> includes the following information.
...
You are familiar with the culture requirements for growth of <i>Neisseria</i> spp. in culture, and the range of [UK Standards for Microbiology Investigations (UK SMI) for Screening for <i>Neisseria</i>](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/343978/B_51i2.pdf).

```{r Neisseria-plate, echo=FALSE, fig.cap="Neisseria meningiditis colonies on a Blood Agar plate. Image credit: [CDC](https://www.cdc.gov/meningitis/lab-manual/chpt07-id-characterization-nm.html)"}
knitr::include_graphics(rep("images/Neisseria-BloodAgar.gif"))
```

You are also familiar with the literature surrounding <i>Neisseria meningitidis</i> identification, and often read papers such as:
...
You are one of the doctors at the largest surgery in `r city_name`. In the past fortnight, `r sample(50:100, size=1)` of your patients have recently presented to the surgery with symptoms consistent with meningiditis. You have therefore recently been familiarising yourself with the relevant literature:
...
You are an epidemiologist working at the University of `r city_name`, and have been studying <i>N. meningitidis</i> transmission dynamics for the past `r sample(2:10, size=1)` years. As such, you are very familiar with the relevant literature, including papers such as: 
