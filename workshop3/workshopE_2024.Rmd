...
9.  Epidemiologist from the local university, an expert on <i>L. monocytogenes</i> transmission
...
```{r A4, fig.cap="Public expenditures by category for the 2022-2023 financial year (Source: Rosmakirk City Council)", echo=FALSE}
...
You are an epidemiologist working for Public Health Scotland, sent to `r city_name` to help with the <i>Listeria</i> outbreak.

A team of scientists working together have in a PHS lab have isolated and cultured <i>Listeria</i> (following standard procedures) from a number of patient samples taken during this outbreak; isolated DNA from these cultures; sequenced this DNA using an Illumina MiSeq platform; assembled the short paired-end reads; and compared these assemblies to the extant <i>Listeria</i> genome sequences available in the NCBI databases. The results of these experiments are summarised in the table below. 
...
You are familiar with the culture requirements for growth of <i>Listeria monocytogenes</i>, and the [UK SMI for identification of <i>Listeria</i> species](https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/889369/UK_SMI_ID_03i4.1.pdf).
...
You are also familiar with the literature surrounding <i>Listteria monocytogenes</i> identification, and often read papers such as:
...
You are an epidemiologist working at the University of `r city_name`, and have been studying <i>L. monocytogenes</i> transmission dynamics for the past `r sample(2:10, size=1)` years. As such, you are very familiar with the relevant literature, including papers such as: 
