...
outbreak_deaths <- floor(runif(1, min = 10, max = 50))
city_name <- "Rosmakirk" 
...
The cause of the outbreak appears to be <i>Listeria monocytogenes</i>. (Please refer to the [WHO Listeriosis Fact Sheet](https://www.who.int/news-room/fact-sheets/detail/listeriosis) for more information about the pathogen.)
...
knitr::include_graphics(rep("images/Listeria.jpg"))
```
...
```{r city, echo=FALSE, fig.cap="Photograph of Rosmakirk city centre. Image credit: DALL-E"}
knitr::include_graphics(rep("images/Rosmakirk.png"))
```
...
```{r A1, fig.show="hold", out.width="50%", echo=FALSE, fig.cap="Rosmakirk demographics (Source: Office for National Statistics)"}
...
ggplot(data = population, aes(x=PopYears, y=Pop)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of Rosmakirk residents (estimated)", x = "Year") +
...
ggplot(data = birthdeath, aes(x=DemogYears, y=Num, group=Demog, colour=Demog)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of births and deaths in Rosmakirk (estimated)", x = "Year", colour = "Demographics") +
  scale_y_continuous(limits=c(0, 5500)) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_color_discrete(name="")
```
You were elected as a member of the popular "Peace and Change" party and made a number of election promises regarding a "Green New Deal" for `r city_name`.

```{r A2, fig.cap="Voting intention by Rosmakirk residents (Source: YouGov polling)", echo=FALSE}

Party <- c((rep("Sovereign Democrat", 10)), (rep("Future First", 10)), (rep("Peace and Change", 10)), (rep("Rational Change", 10)), (rep("Gold", 10)))
...
Parties <- c("Sovereign Democrat", "Future First", "Peace and Change", "Rational Change", "Gold")
...
```{r A4, fig.cap="Public expenditures by category for the 2021-2022 financial year (Source: Rosmakirk City Council)", echo=FALSE}
...
You are a representative from Public Health Scotland, sent to `r city_name` to help with the <i>Listeria</i> outbreak. 
...
REPLACE THIS TEXT and TABLE - A team of scientists working together have in a PHS lab have isolated and cultured <i>Listeria</i> virus (using a standard tissue culture model) from a number of patient samples taken during this outbreak; isolated <i>Listeria</i> RNA from these cultures; reverse-transcribed this to DNA and sequenced this DNA using an Illumina MiSeq platform; assembled the short paired-end reads; and compared these assemblies to the extant <i>Listeria</i> sequences available in the NCBI databases. The results of these experiments are summarised in the table below. 
...
```{r hospital, echo=FALSE, fig.cap="The Horizon NHS hospital in Rosmakirk. Image credit: [DALL-E"}
knitr::include_graphics(rep("images/Horizon-NHS.png"))
```
...
```{r A6, fig.cap="ICU bed occupancy for the Rosmakirk Horizon NHS hospital (Source: NHS Scotland)", echo=FALSE}
...
ggplot(data = ICUstats, aes(x=ICUdates, y=ICU, group = 1)) + 
  geom_point() +
  geom_line() +
  labs(y = "Average Horizon NHS ICU bed occupancy/month", x = "Month") +
...
```{r A7, fig.cap="Admissions to the Rosmakirk Horizon NHS hospital by category (Source: NHS Scotland)", echo=FALSE}
...
```{r A8, fig.cap="ICU waiting times for the Rosmakirk Horizon NHS hospital (Source: NHS Scotland)", echo=FALSE}
...
```{r A9, fig.cap="Key Performance Indicators (KPIs) related to infectious diseases for the Rosmakirk Horizon NHS hospital (Source: NHS Scotland)", echo=FALSE}
...
There are currently `r sample(143:180, size=1)` patients in the `r hospital_name` with confirmed or suspected Listeriosis, with `r sample(15:17, size=1)` of these patients currently in the ICU. 
...
Your briefing packet on <i>Listeria</i> includes the following information.
...
You are familiar with the culture requirements for growth of <i>Listeria monocytogenes</i> in tissue culture, and the range of [ testing methods](link).
...
knitr::include_graphics(rep("images/Listeria-BloodAgar.jpg"))
```
...
You are one of the doctors at the largest surgery in `r city_name`. In the past fortnight, `r sample(50:100, size=1)` of your patients have recently presented to the surgery with symptoms consistent with Listeriosis. You have therefore recently been familiarising yourself with the relevant literature:
