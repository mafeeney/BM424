...
The cause of the outbreak appears to be Influenza A virus. (Please refer to the [WHO Influenza Fact Sheet](https://www.who.int/news-room/fact-sheets/detail/influenza-(seasonal)) for more information about the pathogen.)
...
knitr::include_graphics(rep("images/influenza.jpg"))
```
...
## Information Packet B1

You are the provost of `r city_name`, elected in `r sample(2012:2021, size=1)`. 

```{r city, echo=FALSE, fig.cap="Photograph of Baltyre city centre. Image credit: DALL-E"}
knitr::include_graphics(rep("images/Baltyre.png"))
```
...
```{r A1, fig.show="hold", out.width="50%", echo=FALSE, fig.cap="Baltyre demographics (Source: Office for National Statistics)"}
...
ggplot(data = population, aes(x=PopYears, y=Pop)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of Baltyre residents (estimated)", x = "Year") +
...
ggplot(data = birthdeath, aes(x=DemogYears, y=Num, group=Demog, colour=Demog)) + 
  geom_point() +
  geom_line() +
  labs(y = "Number of births and deaths in Baltyre (estimated)", x = "Year", colour = "Demographics") +
  scale_y_continuous(limits=c(0, 5500)) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_color_discrete(name="")
```
You were elected as a member of the popular "New Progress" party and made a number of election promises regarding a "Green New Deal" for `r city_name`.

```{r A2, fig.cap="Voting intention by Baltyre residents (Source: YouGov polling)", echo=FALSE}

Party <- c((rep("Gold", 10)), (rep("Vocal Action", 10)), (rep("New Progress", 10)), (rep("Truth to Power", 10)), (rep("Social Redistribution", 10)))
...
Parties <- c("Gold", "Vocal Action", "New Progress", "Truth to Power", "Social Redistribution")
...
```{r A4, fig.cap="Public expenditures by category for the 2021-2022 financial year (Source: Baltyre City Council)", echo=FALSE}

expenses <- data.frame(
  Category=c("Education", "Industry and Agriculture", "Social Care", "Transport", "Public order and safety", "Healthcare", "Housing", "Other"),
  value=c(8,11,7,5,6,24,9,1)
)
...
## Information Packet B2

You are a representative from Public Health Scotland, sent to `r city_name` to help with the influenza outbreak. 
...
```{r hospital, echo=FALSE, fig.cap="The Queen's College hospital in Baltyre. Image credit: [DALL-E"}
...
```{r A6, fig.cap="ICU bed occupancy for the Baltyre Queen's College hospital (Source: NHS Scotland)", echo=FALSE}
...
ggplot(data = ICUstats, aes(x=ICUdates, y=ICU, group = 1)) + 
  geom_point() +
  geom_line() +
  labs(y = "Average Queen's College ICU bed occupancy/month", x = "Month") +
...
```{r A7, fig.cap="Admissions to the Baltyre Queen's College hospital by category (Source: NHS Scotland)", echo=FALSE}
...
```{r A8, fig.cap="ICU waiting times for the Baltyre Queen's College hospital (Source: NHS Scotland)", echo=FALSE}
...
```{r A9, fig.cap="Key Performance Indicators (KPIs) related to infectious diseases for the Baltyre Queen's College hospital (Source: NHS Scotland)", echo=FALSE}
...
There are currently `r sample(143:180, size=1)` patients in the `r hospital_name` with confirmed or suspected influenza, with `r sample(15:17, size=1)` of these patients currently in the ICU. 
...
<table border="1">
  ...
  <tr>
    <td>Schools</td>
    <td>4 primary schools, 2 secondary school, 1 college</td>
  </tr>
  <tr>
    <td>Care Homes</td>
    <td>3, each with ~40 bed capacity</td>
  </tr>
  <tr>
...
You are familiar with the culture requirements for growth of influenza viruses in tissue culture, and the range of [influenza virus testing methods](https://www.cdc.gov/flu/professionals/diagnosis/table-testing-methods.htm).
...
knitr::include_graphics(rep("images/influenza-plaqueassay.jpg"))
```
...
## Information Packet B8

You are one of the doctors at the largest surgery in `r city_name`. In the past fortnight, `r sample(50:100, size=1)` of your patients have recently presented to the surgery with symptoms consistent with influenza. You have therefore recently been familiarising yourself with the relevant literature:

[Weekly national respiratory infection and COVID-19 statistical report (PHS)](https://publichealthscotland.scot/publications/weekly-national-respiratory-infection-and-covid-19-statistical-report/weekly-national-respiratory-infection-and-covid-19-statistical-report-18-january-2023/)
